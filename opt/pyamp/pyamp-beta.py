...
class EnhancedList(QListWidget):
    ...
    def show_context_menu(self, position):
        menu = QMenu()
        remove_action = QAction("Listeden Sil (Del)", self)
...
class PyAmp(QMainWindow):
    ...
    def apply_styles(self):
        self.setStyleSheet("""
            ...
            QListWidget::item:selected { background-color: #111; border-left: 2px solid #0f0; }
        """)

    # --- YENİ MOD FONKSİYONLARI ---
    ...
    def status_manager(self, status):
        if status == QMediaPlayer.MediaStatus.EndOfMedia:
            if self.is_repeat:
                self.play_sel() # Aynı şarkıyı tekrar çal
